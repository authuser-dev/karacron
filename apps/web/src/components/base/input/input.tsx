...
export interface InputBaseProps extends Omit<AriaInputProps, 'size'> {
	...
	isRequired?: boolean;
	/** Shows a loading spinner in the trailing position. */
	isLoading?: boolean;
	/**
	 * Input size.
	 * @default "sm"
...
export const InputBase = ({
	...
	const hasTrailingIcon = tooltip || isInvalid || isLoading;
		...
		>
			...
			{isLoading && type !== 'password' && (
				<svg
					aria-label="Loading"
					className={cx(
						'pointer-events-none absolute animate-spin text-fg-brand-primary',
						'size-4',
						sizes[inputSize].iconTrailing,
					)}
					viewBox="0 0 32 32"
