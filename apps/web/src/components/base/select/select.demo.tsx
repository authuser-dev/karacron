...
import { TagSelect } from '@base/select/tag-select';
import { Dot } from '@foundation/dot-icon';
...
const items: SelectItemType[] = [
	...
	},
	{
		...
		avatarUrl:
			'https://www.untitledui.com/images/avatars/abraham-baker?fm=webp&q=80',
	},
	{
		label: 'Adem Lane',
		id: '@adem',
		supportingText: '@adem',
		avatarUrl:
			'https://www.untitledui.com/images/avatars/adem-lane?fm=webp&q=80',
	},
	{
		label: 'Jackson Reed',
		id: '@jackson',
		...
		avatarUrl:
			'https://www.untitledui.com/images/avatars/jackson-reed?fm=webp&q=80',
	},
	{
		label: 'Jessie Meyton',
		id: '@jessie',
		...
		avatarUrl:
			'https://www.untitledui.com/images/avatars/jessie-meyton?fm=webp&q=80',
	},
];
		...
		<div className="flex flex-col gap-4">
			<Select
				...
				items={items.map(({ avatarUrl: _, ...item }) => ({
					...item,
					icon: (
						<Dot className="size-2.5 text-fg-success-secondary" />
					),
				}))}
			>
				{(item) => (
...
const getSelectedUserCount = (selectedKeys: Selection) => {
	if (selectedKeys === 'all')
		return teamItems.reduce(
			(sum, t) => sum + parseInt(t.supportingText?.split(' ')[0] || '0'),
			0,
		);
	const selected = teamItems.filter((t) =>
		(selectedKeys as Set<string | number>).has(t.id),
	);
...
export const MultiSelectSmDemo = () => {
	...
	return (
		<MultiSelect
			...
			onSelectAll={() =>
				setSelectedKeys(new Set(teamItems.map((t) => t.id)))
			}
		>
			{(item) => (
				<MultiSelect.Item
