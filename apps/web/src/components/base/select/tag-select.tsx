...
import { type SelectItemType, sizes } from '@base/select/select-shared';
import { TagCloseX } from '@base/tags/base-components/tag-close-x';
import { useResizeObserver } from '@hook/use-resize-observer';
...
const InnerTagSelect = ({
	...
	const isSelectionEmpty =
		tagSelectContext?.selectedItems?.items?.length === 0;
				...
				tagSelectContext?.selectedItems?.items?.map((value) => (
					...
					>
						...
						<TagCloseX
							...
							onPress={() =>
								tagSelectContext.onRemove(new Set([value.id]))
							}
						/>
					</span>
				))}
