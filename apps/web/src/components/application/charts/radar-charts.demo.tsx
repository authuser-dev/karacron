	...
	useEffect(() => {
		if (textRef.current && rectRef.current) {
			...
			rectRef.current.setAttribute(
				'width',
				(textBoundingBox.width + EXTRA_WIDTH).toString(),
			);
			rectRef.current.setAttribute(
				'height',
				(textBoundingBox.height + EXTRA_HEIGHT).toString(),
			);
			rectRef.current.setAttribute(
				'x',
				(textBoundingBox.x - EXTRA_WIDTH / 2).toString(),
			);
			rectRef.current.setAttribute(
				'y',
				(textBoundingBox.y - EXTRA_HEIGHT / 2).toString(),
			);
		}
	}, [x, y]);
	...
	return (
		<Fragment>
			<rect
				...
				y={y}
				rx={11}
				className="fill-utility-neutral-50 stroke-border-secondary_alt stroke-1"
			></rect>
			<text
				ref={textRef}
				x={x}
			...
			>
				<tspan
					x={x}
					dy="0em"
					className="fill-utility-neutral-700 text-xs font-medium"
				>
					{payload.value}
				</tspan>
			</text>
